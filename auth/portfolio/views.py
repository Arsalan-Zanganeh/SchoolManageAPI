...
class StudentPictureTeacherSideView(APIView):
    def post(self, request):
        ...
        if not myclass:
            return Response({"message":"There is no class with this ID"})

        students1 = ClassStudent.objects.filter(Classes=myclass, Student__National_ID=request.data['National_ID']).first()

        if not students1:
            raise AuthenticationFailed("There is no student with this National_ID in your class")

        profPic = StudentProfile.objects.filter(student=students1.Student).first()
